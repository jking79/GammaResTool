...
//
GammaResToolAod::GammaResToolAod(const edm::ParameterSet& iConfig) :
...
// -- consume tags ------------------------------------------------------------
	...
	// photons
	gedPhotonsToken_ = consumes<phoColView>(gedPhotonsTag);
	ootPhotonsToken_ = consumes<phoColView>(ootPhotonsTag);
...
GammaResToolAod::~GammaResToolAod(){
	///////////////////////////////////////////////////////////////////
...
float GammaResToolAod::getPhotonSeedTime( reco::Photon photon ){
...
}//<<>>float GammaResTool::getPhotonSeedTime( reco::Photon )

int GammaResToolAod::getRhIdx( uInt rhDetID, std::vector<uInt> rhID ){
...
void GammaResToolAod::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
...
// -- Process ECAL RecHits
	...
	float minRecHitEnergy = 2.0;
	if( DEBUG ) std::cout << "Processing RecHits" << std::endl;
	for (const auto & recHit : *recHitsEB_ ){ if( recHit.energy() > minRecHitEnergy ) frechits.push_back(recHit); frhUsed.push_back(false);}
    for (const auto & recHit : *recHitsEE_ ){ if( recHit.energy() > minRecHitEnergy ) frechits.push_back(recHit); frhUsed.push_back(false);}
    //if( doDiag ){
...
// -- Process gedPhotons
    ...
    for( const auto & photon : *gedPhotons_ ){
		...
		bool hoe = photon.hadronicOverEm() < 0.04596;
		bool trkiso = photon.trkSumPtHollowConeDR04() < 1.694;
		bool ecaliso = photon.hcalTowerSumEtConeDR04() < 24.032 + 0.01512*pho_pt + 2.259e-05*pho_pt*pho_pt;
		bool hcaliso = photon.ecalRecHitSumEtConeDR04() < 2.876 + 0.004017*pho_pt;
        bool passIdCut = hoe && trkiso && ecaliso && hcaliso;
		bool minPhoPt = pho_pt > phoMinPt;
		float phoSeedTime = getPhotonSeedTime(photon);
		bool timecut = phoSeedTime > phoMinSeedTime;
...
// -- Process electrons
    ...
    //std::vector<reco::Electron> gloPhotons;
    ...
    if( DEBUG ) std::cout << "Found " << felectrons.size() << " electrons" << std::endl;
...
*/
    ...
    for (const auto & recHit : frechits ){
    	...
    	if( doDiag ){
        	if( recHit.energy() > 2.0 ){ // energy cut to restrict to energies used in resolutions and spike rejection plots

				rhIsUsed.push_back(false);
...
/*
    ...
    if( doDiag ){
		...
		if( recHit.energy() > 5.0 ){ // energy cut to restrict to energies used in resolutions and spike rejection plots
...
*/
        ...
        if ( smin < 0.3 && smaj < 0.5){
			...
			for( auto & rechit : phoRhGroup ){
				...
				for( auto & offset : offsets ){ 
					...
					bool ordered = lrhEnergy > neighborEnergy;
					bool close = lrhEnergy < ( 1.20 * neighborEnergy );
					bool minRhE = ( lrhEnergy > 0.5 ) && ( neighborEnergy > 0.5 ); 
					//if( DEBUG ) std::cout << " Examining rechit pair with " << lrhEnergy << " & "; 
					//if( DEBUG ) std::cout << neighborEnergy << " energies" << std::endl;
					if( ordered && close && minRhE ){  // need to be within 20% of energy
						if( DEBUG ) std::cout << " Matching loc rechit pair with ev: " << lrhEnergy << " : " << neighborEnergy;
                        if( DEBUG ) std::cout << " (" << lrhEnergy/neighborEnergy << ")"<< " for lead rh id: "; 
                        if( DEBUG ) std::cout << rhDetId.rawId() <<std::endl;
	...
	for( auto & photon : selPhotons ){
		...
		for( auto phorhid : phoRhIdsGroup ){ for( int i = 0; i < nRhIds; i++ ){ if( rhID[i] == phorhid ){ rhIsUsed[i] = true; break; } } }
        const auto seedDetId = scptr->seed()->seed(); // seed detid
        const auto isEB = (seedDetId.subdetId() == EcalSubdetector::EcalBarrel); // which subdet
        const auto phoRecHits = (isEB ? *recHitsEB_ : *recHitsEE_ );
...
void GammaResToolAod::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(GammaResToolAod);
