...
//---------fitter.MakeTimeFits()------ run script substances class with <TimeFitter fitter;> then runs <fitter.MakeTimeFits();> -------------------
  ...
  //auto & GFitInfo.ResultsMap = GFitInfo.FitInfo.ResultsMap;
  // setup hists
  const auto xbins = &fXBins[0];
  //TH1F * TimeFitter::SetupHist(const TString & ytitle, const TString & yextra, const TString & label) 
  //FitInfo.ResultsMap["chi2ndf"]  = TimeFitter::SetupHist("#chi^{2}/NDF","chi2ndf",label);
  ...
  //GFitInfo.ResultsMap["chi2ndf"]  = new TH1F(gchi2ndfName.c_str(),chi2ndfName.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["chi2ndf"]->Sumw2();
  //FitInfo.ResultsMap["chi2prob"] = TimeFitter::SetupHist("#chi^{2} Prob.","chi2prob",label);
  ...
  //GFitInfo.ResultsMap["chi2prob"] = new TH1F(gchi2probName.c_str(),chi2probTitle.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["chi2prob"]->Sumw2();
  auto mutext = "#mu("+fTimeText+") [ns]";
  auto muName = inplotname+"_mu";
  //auto gmuName = glabel+"_mu";
  auto muTitle = fTitle+" "+mutext+";"+fXTitle+";"+mutext;
  //FitInfo.ResultsMap["mu"]       = TimeFitter::SetupHist(mutext,"mu",label);
  ResultsMap["mu"] = new TH1F(muName.c_str(),muTitle.c_str(),fNBinsX,xbins);
  ResultsMap["mu"]->Sumw2();
  //GFitInfo.ResultsMap["mu"] = new TH1F(gmuName.c_str(),muTitle.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["mu"]->Sumw2();
  ...
  //FitInfo.ResultsMap["sigma"]    = TimeFitter::SetupHist(sigtext,"sigma",label);
  ResultsMap["sigma"] = new TH1F(sigName.c_str(),sigTitle.c_str(),fNBinsX,xbins);
  ResultsMap["sigma"]->Sumw2();
  //GFitInfo.ResultsMap["sigma"] = new TH1F(gsigName.c_str(),sigTitle.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["sigma"]->Sumw2();
  ...
  //FitInfo.ResultsMap["occ"]       = TimeFitter::SetupHist(occtext,"occ",label);
  ResultsMap["occ"] = new TH1F(occName.c_str(),occTitle.c_str(),fNBinsX,xbins);
  ResultsMap["occ"]->Sumw2();
  //GFitInfo.ResultsMap["occ"] = new TH1F(goccName.c_str(),occTitle.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["occ"]->Sumw2();
  ...
  //FitInfo.ResultsMap["rms"]       = TimeFitter::SetupHist(occtext,"occ",label);
  ResultsMap["rms"] = new TH1F(rmsName.c_str(),rmsTitle.c_str(),fNBinsX,xbins);
  ResultsMap["rms"]->Sumw2();
  //GFitInfo.ResultsMap["rms"] = new TH1F(grmsName.c_str(),rmsTitle.c_str(),fNBinsX,xbins);
  //GFitInfo.ResultsMap["rms"]->Sumw2();
  ...
  for (auto ibinX = 1; ibinX < fNBinsX; ibinX++){ 
    ...
    //GFitInfo.ResultsMap["chi2ndf"] ->SetBinContent(ibinX,gresult.chi2ndf);
    //GFitInfo.ResultsMap["chi2prob"]->SetBinContent(ibinX,gresult.chi2prob);
    ResultsMap["mu"]->SetBinContent(ibinX,result.mu);
    ResultsMap["mu"]->SetBinError(ibinX,result.emu);
    //GFitInfo.ResultsMap["mu"]      ->SetBinContent(ibinX,gresult.mu);
    //GFitInfo.ResultsMap["mu"]      ->SetBinError  (ibinX,gresult.emu);
    ResultsMap["sigma"]->SetBinContent(ibinX,result.sigma-estTOF);
    ResultsMap["sigma"]->SetBinError(ibinX,result.esigma);
    //GFitInfo.ResultsMap["sigma"]   ->SetBinContent(ibinX,gresult.sigma-estTOF);
    //GFitInfo.ResultsMap["sigma"]   ->SetBinError  (ibinX,fserr);
    ResultsMap["occ"]->SetBinContent(ibinX,result.occ);
    ResultsMap["occ"]->SetBinError(ibinX,sqrt(result.occ));
    //GFitInfo.ResultsMap["occ"]   ->SetBinContent(ibinX,gresult.occ);
    //GFitInfo.ResultsMap["occ"]   ->SetBinError  (ibinX,sqrt(gresult.occ));
    ResultsMap["rms"]->SetBinContent(ibinX,result.rms);
    ResultsMap["rms"]->SetBinError(ibinX,err);
    //GFitInfo.ResultsMap["rms"]   ->SetBinContent(ibinX,gresult.rms);
    //GFitInfo.ResultsMap["rms"]   ->SetBinError  (ibinX,err);
  ...
  // get input (FitInfo.ResultsMap["sigma"])
  //const auto & (FitInfo.ResultsMap["sigma"]) = FitInfo.ResultsMap["sigma"];
  // get range
  const auto x_low = ResultsMap["sigma"]->GetXaxis()->GetBinLowEdge(ResultsMap["sigma"]->GetXaxis()->GetFirst());
  const auto x_up  = ResultsMap["sigma"]->GetXaxis()->GetBinUpEdge(ResultsMap["sigma"]->GetXaxis()->GetLast());
  // set names
  std::cout << "Prepping sigma form for: " << label << std::endl;
  const std::string histname = ResultsMap["sigma"]->GetName();
  ...
  // set line color
  fit->SetLineColor(ResultsMap["sigma"]->GetLineColor());
  // save form
  std::cout << "Saving sigma form for: " << sflabel << std::endl;
  fOutFile->cd();
  form->Write(form->GetName(),TObject::kWriteDelete);

  // Fit sigma (FitInfo.ResultsMap["sigma"])
  //---------------------TimeFitter::FitSigmaHist(FitInfo);
  //const auto & label = FitInfo.label;
  std::cout << "Fitting sigma hist for: " << label << std::endl;
  // get inputs/outputs
  //auto & (FitInfo.ResultsMap["sigma"]) = FitInfo.ResultsMap["sigma"];
  //auto & fit  = FitInfo.SigmaFit;
  // and fit it
  //auto c1 = new TCanvas( "c1", "canvas" , 200, 10, 700, 500 );
  //c1->SetBatch(true);
  //(FitInfo.ResultsMap["sigma"])->Draw();
  //(FitInfo.ResultsMap["sigma"])->Fit(fit->GetName(),"RBQ0");
  ResultsMap["sigma"]->Fit(fit->GetName(),"RBQM");
  //c1->Modified();
  //c1->Update();
  //c1->Draw();
  // save to output
